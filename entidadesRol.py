...
class Prestamo:
    def __init__(self,empleado, aamm, valor, numPagos,saldo, estado= True,id=1):
        ...
        self.cuota = int(valor)/int(numPagos)
        self.saldo = saldo
        self.estado = estado
...
class DetalleNomima(CalculoRol):
    ...
    def getSobretiempo(self,aamm):
        ...
        if self.empleado.id[0]=="O":
            ...
            if sobretiempo:
                entSobretiempo = Sobretiempo(sobretiempo[1],sobretiempo[2],float(sobretiempo[3]),float(sobretiempo[4]),id=sobretiempo[0])
                calSob = round(self.empleado.valorHora()+(entSobretiempo.h50*1.5+entSobretiempo.h100*2),2) 
        return calSob

    def getAntiguedad(self,deduccion,nomina):
        calAnt = 0
        if self.empleado.id[0]=="O":
            calAnt = round(deduccion.getAntiguedad()*(nomina.year-int(self.empleado.fechaIngreso[:4]))/365*self.empleado.sueldo,2)
        return calAnt
    ...
    def calcularRubrosIngresos(self,aamm,deduccion):
        ...
        ingresos.append(self.getAntiguedad(deduccion,date.today()))
